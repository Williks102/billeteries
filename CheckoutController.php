    ...
    public function process(Request $request)
    {
        ...
        try {
            ...
            foreach ($eventGroups as $eventId => $items) {
                ...
                // ✅ CORRECTION 1: Commission avec bon nom de champ
                $this->createCommissionForOrder($order);
            }

            // ✅ CORRECTION 2: Envoyer les emails POUR TOUTES LES COMMANDES
            $this->sendEmailsForAllOrders($orders);
...
private function createCommissionForOrder($order)
{
    ...
    Commission::create([
        'order_id' => $order->id,
        'promoter_id' => $order->event->promoteur_id,  // ✅ CORRIGÉ
     ...
     * ✅ CORRECTION 2: Envoyer les emails pour TOUTES les commandes
     */
    private function sendEmailsForAllOrders($orders)
    {
        $emailService = app(\App\Services\EmailService::class);
        
        foreach ($orders as $order) {
            try {
                // Envoyer tous les emails pour cette commande
                $emailService->sendAllOrderEmails($order);
                
                \Log::info('Emails envoyés pour la commande', [
                    'order_id' => $order->id,
                ...
                \Log::error('Erreur envoi emails pour commande', [
                    'order_id' => $order->id,
                    'error' => $e->getMessage()
                ]);
                // Ne pas faire échouer la transactions pour un problème d'email
            }
        }
    }
}